...
- 支持 **图像、文本、音频、视频、装置或混合** 媒介。

可以说：*帮我 critique 一下*、*review 我的作品*、*你对这件作品有什么感受* 等。

### 使用

安装后让 Agent 能读取 skill 根目录下的 `SKILL.md`（路径因运行环境而异）。与 Agent 对话时，会在工作目录创建 session 文件夹（如 `art-critique-sessions/...`），不在 skill 目录内写入。
...
- Critique **image, text, audio, video, installation, or mixed** work.

Say things like: *critique this*, *help me review my artwork*, *what do you feel about this piece*.

### Setup

Install so your agent can see `SKILL.md` at the skill root (path depends on the runtime). Run a critique from your project folder; the agent creates a session directory (e.g. `art-critique-sessions/...`) and works there — not inside the skill folder.
